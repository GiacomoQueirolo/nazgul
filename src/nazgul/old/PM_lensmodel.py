...
while True:
    RndGal = get_rnd_gal(check_prev=True)
    if len(RndGal.stars["mass"])<1 or len(RndGal.gas["mass"])<1:
        # ignore gal without stars or gas
        continue
    Gal = RndGal 
...
# centered around 0
# convert in arcsec
arcXkpc = cosmo.arcsec_per_kpc_proper(sett.z_lens)

Xstar,Ystar,_ =  Gal.stars["coords"] - Gal.center # Mpc
#RAstar,DECstar = Xstar.to("kpc").value*arcXkpc,Ystar.to("kpc").value*arcXkpc
Xgas,Ygas,_ =  Gal.gas["coords"] - Gal.center # Mpc
#RAgas,DECgas = Xgas.to("kpc").value*arcXkpc,Ygas.to("kpc").value*arcXkpc
Xdm,Ydm,_ =  Gal.dm["coords"] - Gal.center # Mpc
#RAdm,DECdm = Xdm.to("kpc").value*arcXkpc,Ydm.to("kpc").value*arcXkpc
Xbh,Ybh,_ =  Gal.bh["coords"] - Gal.center # Mpc
